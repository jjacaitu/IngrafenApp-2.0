...
from cotizacion.forms import Crear_usuario, Solicitud_cot, Materiales, Clientes, TipoDeTrabajo, Solicitud_ot, Solicitud_ot_aprobacion, Clientes_ot #Bloqueo_cliente
from django.contrib.auth import authenticate, login, logout
from django.http import HttpResponse, HttpResponseRedirect
from django.urls import reverse
from django.contrib.auth.decorators import login_required
from cotizacion.models import CotizacionesSolicitadas, OrdenesSolicitadas
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'home.html', {"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cotizaciones_existentes":cotizaciones_existentes})
...
def error01(request):
    ...
    return render(request, "error01.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cotizaciones_existentes":cotizaciones_existentes})
...
def contactenos(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, "contactanos.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cotizaciones_existentes":cotizaciones_existentes})
...
def quienes_somos(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, "compañia.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cotizaciones_existentes":cotizaciones_existentes})
...
def servicios(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, "servicios.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_existentes":ordenes_existentes,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"cotizaciones_existentes":cotizaciones_existentes})

def solicitud(request):
    cotizaciones_existentes = CotizacionesSolicitadas.objects.all().filter(cotizador__exact="")
    ordenes_existentes = OrdenesSolicitadas.objects.all().filter(cotizador_ot__exact="")
    return render( request, "solicitud.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cotizaciones_existentes":cotizaciones_existentes})
...
def creacion_usuario(request):
    ...
    return render(request, 'registro.html', {"ordenes_existentes":ordenes_existentes,'usuario': usuario, "registered":registered,"cotizaciones_existentes":cotizaciones_existentes})
    ...
    if request.method == "POST" and request.POST.get("reutilizar") == "REUTILIZAR ULTIMA COTIZACION":
        ...
        if cotizacion.is_valid():
            ...
            if request.POST.get("impresionr4"):
                stock.impresion_retiro4 = request.POST.get("num_pantonesr4") + " " + request.POST.get("impresionr4")
            ...
            if request.POST.get("impresionr1"):
                stock.impresion_retiro5 = request.POST.get("num_pantonesr5") + " " + request.POST.get("impresionr5")
...
def solicitudes_existentes(request):
    ...
    if request.method == "GET":
        ...
        return render(request, "solicitudes_existentes.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cotizaciones_existentes":cotizaciones_existentes,"buscar":buscar})
    elif request.method == "POST" and request.POST.get("boton_regresar") == "REGRESAR":
        buscar=False
        if request.user.categoria == "VEN":
            cotizaciones_existentes = CotizacionesSolicitadas.objects.all().filter(cotizador__exact="").filter(vendedor=request.user)
        ...
        #return render(request,"solicitudes_existentes.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"cotizacion_completada":cotizacion_completada,"buscar":buscar,"numero_a_ver":numero_a_ver})
...
def cotizaciones_completadas(request):
    ...
    if request.method == "GET" and request.user.categoria == "VEN":
        ...
        return render(request,"cotizaciones_completadas.html",{"desde":desde,"hasta":hasta,"tipo_busqueda":tipo_busqueda,"busqueda":busqueda,"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cotizaciones_completadas":cotizaciones_completadas,"ver":ver,"clientes_creados":clientes_creados,"trabajos_creados":trabajos_creados,"cotizadores":cotizadores,"vendedores":vendedores, "cotizaciones_existentes":cotizaciones_existentes})
...
def creacion_material(request):
    ...
    if request.method == "POST" and request.POST.get("crear") == "CREAR":
        ...
        if material.is_valid():
            ...
            material = Materiales()
    return render(request, "materiales.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"material":material, "creado":creado, "cotizaciones_existentes":cotizaciones_existentes})
...
def creacion_trabajo(request):
    ...
    return render(request, "tipos_trabajo.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"trabajo":trabajo, "creado":creado, "cotizaciones_existentes":cotizaciones_existentes})
...
def creacion_cliente(request):
    ...
    if request.method == "POST" and request.POST.get("crear") == "CREAR":
        ...
        if cliente.is_valid():
            ...
            cliente = Clientes()
    return render(request, "clientes.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cliente":cliente, "creado":creado, "cotizaciones_existentes":cotizaciones_existentes})
...
def creacion_cliente_ot(request):
    ...
    return render(request, "clientes_ot.html",{"ordenes_por_confirmar":ordenes_por_confirmar,"ordenes_proceso":ordenes_proceso,"ordenes_por_fecha":ordenes_por_fecha,"ordenes_existentes":ordenes_existentes,"cliente":cliente, "creado":creado, "cotizaciones_existentes":cotizaciones_existentes})
...
def reportes(request):
    ...
    if request.method == "GET":
        ...
        for vendedor in vendedores:
            ...
            try:
                ven_dict[vendedor.username]["ratio año"] = str(round(((OrdenesSolicitadas.objects.all().filter(vendedor_ot=vendedor.username).filter(fecha_completada_ot__range=[primero_año,(primero_año + relativedelta(years=+1))]).exclude(cotizador_ot="").count())/(CotizacionesSolicitadas.objects.all().filter(vendedor=vendedor.username).filter(fecha_completada__range=[primero_año,(primero_año + relativedelta(years=+1))]).exclude(cotizador="").count())),2)*100) + "%"
            except ZeroDivisionError:
    ...
    if request.method == "POST" and request.POST.get("buscar") == "BUSCAR":
        ...
        for cotizador in cotizadores:
            ...
            try:
                cot_dict[cotizador.username]["ratio"] = str(round(((OrdenesSolicitadas.objects.all().filter(cotizador_ot=cotizador.username).filter(fecha_completada_ot__range=[desde,hasta]).exclude(cotizador_ot="").count())/(CotizacionesSolicitadas.objects.all().filter(cotizador=cotizador.username).filter(fecha_completada__range=[desde,hasta]).exclude(cotizador="").count())),2)*100) + "%"
            except ZeroDivisionError:
        ...
        try:
            cot_dict["Total"]["ratio"] = str(round(((OrdenesSolicitadas.objects.all().filter(fecha_completada_ot__range=[desde,hasta]).exclude(cotizador_ot="").count())/(CotizacionesSolicitadas.objects.all().filter(fecha_completada__range=[desde,hasta]).exclude(cotizador="").count())),2)*100) + "%"
        except ZeroDivisionError:
        ...
        try:
            ven_dict["Total"]["ratio"] = str(round(((OrdenesSolicitadas.objects.all().filter(fecha_completada_ot__range=[desde,hasta]).exclude(cotizador_ot="").count())/(CotizacionesSolicitadas.objects.all().filter(fecha_completada__range=[desde,hasta]).exclude(cotizador="").count())),2)*100) + "%"
        except ZeroDivisionError:
